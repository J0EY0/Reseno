...
function getStableChunkName(manifestKey, manifestEntry) {
  ...
  return manifestEntry.name ?? manifestKey.replace(/-[A-Za-z0-9_-]+\.js$/, '.js')
}
...
function measureClosure(manifest, measurements, manifestKeys) {
  ...
  for (const manifestKey of manifestKeys) {
    ...
    if (!measurement) {
      throw new Error(`Missing measurement for emitted JavaScript file: ${file}`)
    }
...
async function verifyBundleBudgets() {
  ...
  try {
    ...
    const measurements = await measureJavaScriptFiles(temporaryOutput, manifest)
    const failures = []
    ...
    for (const budget of conditionalFontCssBudgets) {
      ...
      if (shellCssFiles.has(entry.file)) {
        failures.push(`${budget.name}: must not be part of the shell CSS closure`)
      }
    ...
    const foundLegacyEntries = new Set()

    console.log('\nFirst-party dynamic entries:')
    for (const entry of dynamicEntries) {
      const legacyBudget = dynamicEntryLegacyBudgets.get(entry.manifestKey)
      const maxRawBytes =
        legacyBudget?.maxRawBytes ?? DEFAULT_DYNAMIC_ENTRY_RAW_BYTES
      const maxGzipBytes =
        legacyBudget?.maxGzipBytes ?? DEFAULT_DYNAMIC_ENTRY_GZIP_BYTES

      if (legacyBudget) {
        foundLegacyEntries.add(entry.manifestKey)
      }

      console.log(
        `  ${entry.manifestKey}: ${formatKiB(entry.rawBytes)} raw / ${formatKiB(entry.gzipBytes)} gzip${legacyBudget ? ' (legacy)' : ''}`,
      )

      if (entry.rawBytes > maxRawBytes) {
        failures.push(
          `dynamic entry ${entry.manifestKey}: raw ${formatKiB(entry.rawBytes)} exceeds ${formatKiB(maxRawBytes)}`,
        )
      }
      if (entry.gzipBytes > maxGzipBytes) {
        failures.push(
          `dynamic entry ${entry.manifestKey}: gzip ${formatKiB(entry.gzipBytes)} exceeds ${formatKiB(maxGzipBytes)}`,
        )
      }
      ...
      const meetsDefaultBudget =
        entry.rawBytes <= DEFAULT_DYNAMIC_ENTRY_RAW_BYTES &&
        entry.gzipBytes <= DEFAULT_DYNAMIC_ENTRY_GZIP_BYTES
      const canRatchetRaw =
        entry.rawBytes <= legacyBudget.maxRawBytes * RATCHET_THRESHOLD
      const canRatchetGzip =
        entry.gzipBytes <= legacyBudget.maxGzipBytes * RATCHET_THRESHOLD

      if (meetsDefaultBudget) {
        failures.push(
          `dynamic entry ${entry.manifestKey}: now meets the default budget; remove its legacy ceiling`,
        )
      } else if (canRatchetRaw || canRatchetGzip) {
        failures.push(
          `dynamic entry ${entry.manifestKey}: is at least 5% below its legacy ${[
            canRatchetRaw ? 'raw' : null,
            canRatchetGzip ? 'gzip' : null,
          ]
            .filter(Boolean)
            .join(' and ')} ceiling; ratchet the exact ceiling down`,
        )
      }
    }

    for (const legacyEntry of dynamicEntryLegacyBudgets.keys()) {
      if (!foundLegacyEntries.has(legacyEntry)) {
        failures.push(
          `dynamic entry ${legacyEntry}: legacy ceiling is stale because the entry is no longer emitted`,
        )
      }
    }
    ...
    for (const budget of routeBudgets) {
      ...
      if (measurement.gzipBytes > budget.maxGzipBytes) {
        ...
        measurement.gzipBytes <= budget.maxGzipBytes * RATCHET_THRESHOLD
      ) {
        failures.push(
