...
async function createConversation({ fetchResource, stopRun, loadActiveRun } = {}) {
  ...
  function load(relativePath, imports = {}) {
    return loadTypeScriptModule(new URL(relativePath, sourceRoot), { context, imports });
  }
  ...
  const runtimeModule = await load(
    "components/copilot/agent-conversation-runtime.ts", { react },
  );
  ...
  const updates = {
    ...
    setSessionLoadError: (value) => { sessionError = value; },
  ...
  const runModule = await load("components/copilot/use-agent-run-stream.ts", imports);
  const consumeRunStream = runModule.useAgentRunStream({
    refreshAgentSession, runtimeRef, updates,
  });
  const hydrationModule = await load("components/copilot/use-agent-session-hydration.ts", {
    ...
    "./copilot-message-model": {
      hydrateAgentSession: async (request) => ({
        session: await request, draftSnapshot: null, panelMessages: [],
      }),
    },
  });
  const retrySession = () => hydrationModule.useAgentSessionHydration({
  ...
  const sendModule = await load("components/copilot/use-agent-send-controller.ts", imports);
  ...
  const promptModule = await load("components/copilot/use-agent-prompt-actions.ts", {
    ...imports,
    "@/lib/agent-attachment-client": {
      uploadAgentAttachment: async () => ({ id: "uploaded-1", filename: "resume.pdf" }),
    },
    "./copilot-attachment-policy": {
      MAX_AGENT_ATTACHMENTS: 5,
      prepareAgentAttachment: async () => ({ body: {}, byteLength: 1 }),
      deletePendingUploads: async (resumeId, files) => deletedUploads.push(...files),
    },
  });
  ...
  const composer = {
    attachments: {
      files: [{ id: "local-1", type: "file", filename: "resume.pdf", url: "data:application/pdf;base64,AA==" }],
      add: () => undefined,
      clear: () => undefined,
      openFileDialog: () => undefined,
      remove: (id) => {
        composer.attachments.files = composer.attachments.files.filter((file) => file.id !== id);
      },
    },
    registerFileInput: () => undefined,
    textInput: {
      value: "Keep my original prompt",
      clear: () => { composer.textInput.value = ""; },
    },
  };
  const formModule = await load("components/ai-elements/use-prompt-input-form.ts", {
    react,
  ...
  return {
    composer, consumeRunStream, deletedUploads, form, prompt, requests, retrySession,
    rollbacks, runtime, send,
    connectRun: (run) => {
      const abortController = new AbortController();
      runtime.activeRequestAbort = abortController;
      return consumeRunStream(
        (options) => streamClient.connectAgentRun(run, options), abortController,
      );
    },
...

{
  ...
  const conversation = await createConversation({
    ...
    loadActiveRun: () => { recoveryReads += 1; return null; },
    stopRun: async () => {
      stops += 1;
  ...
  conversation.prompt.referenceHistoryAttachment({ id: "history-1", filename: "job.pdf" });
  const submitted = conversation.form.handleSubmit({ preventDefault() {}, currentTarget: {} });
  ...
  await submitted;
  await flush();
  assert.equal(conversation.composer.textInput.value, "Keep my original prompt",
    "A POST rejected before acceptance must preserve the submitted text.");
  assert.equal(conversation.composer.attachments.files.length, 1,
  ...
  post = deferred();
  const retried = conversation.form.handleSubmit({ preventDefault() {}, currentTarget: {} });
  while (conversation.requests.length < 2) {
    await flush();
  }
  assert.deepEqual(
    JSON.parse(conversation.requests[1].options.body).message.files.map((file) => file.id),
    ["history-1", "uploaded-1"],
    "Retry must retain both the referenced attachment and the local upload.",
  );
  post.resolve(new Response(new ReadableStream({
  ...
  assert.equal(conversation.runtime.requestPhase, "responding",
...

{
  ...
  let stopCount = 0;
  const run = {
    id: "run-1", status: "active", executionState: "running",
    errorCode: null, baseResume: {}, resumeId: "resume-1",
  };
  const conversation = await createConversation({
    fetchResource: async () => new Response(stopped
      ? 'id: 1\nevent: run_done\ndata: {"status":"cancelled","executionState":"cancelled"}\n\n'
      : "", {
  ...
  assert.equal(conversation.sessionReads, 0,
...
for (const runAlreadyFinished of [false, true]) {
  ...
  const conversation = await createConversation({
    fetchResource: async () => recoveryStarted
      ? new Response(
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    await flush();
    if (conversation.runtime.sessionReady && !conversation.runtime.activeRequestAbort) break;
  }
  assert.equal(conversation.sessionError, false);
  assert.equal(conversation.runtime.requestPhase, "idle");
  assert.equal(conversation.runtime.activeRun, null);
  assert.equal(conversation.requests.length, runAlreadyFinished ? 1 : 2,
