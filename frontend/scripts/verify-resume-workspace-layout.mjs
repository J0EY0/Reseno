...
} from "../src/components/workspace/resume-workspace-layout.ts";
...
for (const containerWidth of [0, 320, 960]) {
  for (const expanded of [false, true]) {
    const resolved = resolveWorkspaceWidths(containerWidth, desired, expanded);
...
try {
  ...
  const savedPreference = values.get(storageKey);
  resolveWorkspaceWidths(1280, readWorkspaceLayoutPreference(), true);
  assert.equal(values.get(storageKey), savedPreference);
...
console.log(
  "Resume workspace width constraints and local preferences verified.",
);
