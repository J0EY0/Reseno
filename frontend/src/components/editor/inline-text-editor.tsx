...
export default function InlineTextEditor({
  ...
  const editor = useEditor(
    {
      ...
      editorProps: {
        ...
        transformPastedHTML: (html) =>
          serializeInlineTextToHtml(
            serializeInlineTextFromHtml(html, multiline),
            multiline,
          ),
  ...
  useEffect(() => {
    if (
      !editor ||
      serializeInlineTextToHtml(
        serializeInlineTextFromHtml(editor.getHTML(), multiline),
        multiline,
      ) === editorValue
    )
      return;
  ...
  return (
    <>
      ...
      {createPortal(
        <div ref={setMenuHost} className="contents" />,
        document.body,
      )}
      {editor && menuHost ? (
        <BubbleMenu
        ...
        <div className={editorClassName} aria-hidden="true">
          &nbsp;
        </div>
      ) : null}
    </>
