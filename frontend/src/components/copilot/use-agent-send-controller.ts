...
import { shouldRollbackOptimisticAgentMessages } from "@/lib/agent-panel-state";
...
export function useAgentSendController({
  ...
  const stopResponding = useCallback(() => {
    ...
    if (activeRun) {
      void stopAgentRun(activeRun.id)
        .then(() => {
          if (runtime.activeRun === activeRun && !runtime.activeRequestAbort) {
            retrySession();
          }
        })
        .catch((error) => {
          ...
          notifyApiError(error, runtime.requestFailedText);
        });
      return;
    }
  ...
  const sendPrompt: SendAgentPrompt = useCallback(
    (
      ...
      const releasePreflight = () => {
        if (preflightOwner && preflightAbortRef.current === preflightOwner) {
          preflightAbortRef.current = null;
        }
        preflightOwner = null;
      };
          ...
          runtime.replyTimer = window.setTimeout(() => {
            ...
            void (async () => {
              ...
              try {
                ...
                if (status === "failed") {
                  notifyApiError(failure, runtime.requestFailedText);
                }
