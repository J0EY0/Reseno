...
function normalizeModelConfig(
  ...
  const temperature =
    typeof raw.temperature === "number" && Number.isFinite(raw.temperature)
      ...
      : null;
  const topP =
    typeof raw.topP === "number" && Number.isFinite(raw.topP) ? raw.topP : null;
  ...
  return createDefaultModelConfig(locale, {
    ...
    provider,
    apiUrl:
      typeof raw.apiUrl === "string" && raw.apiUrl.trim() ? raw.apiUrl : "",
    temperature,
    topP,
    maxTokens: normalizeMaxTokens(raw.maxTokens),
    contextWindowTokens:
      normalizeMaxTokens(raw.contextWindowTokens) ??
      DEFAULT_CONTEXT_WINDOW_TOKENS,
    supportsImage: Boolean(raw.supportsImage),
    supportsThinking: Boolean(raw.supportsThinking),
    thinkingMode: resolveThinkingMode(raw.thinkingMode, availableThinkingModes),
...
export function getModelDisplayName(
  config: Pick<ModelConfig, "nickname" | "model">,
) {
