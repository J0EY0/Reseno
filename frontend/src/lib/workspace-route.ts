...
export type WorkspaceRouteKind = WorkspaceRoute["kind"];
...
export function getWorkspaceRoute(pathname: string): WorkspaceRoute {
  const resumeDetailMatch = matchPath(
    workspaceRoutePaths.resumeDetail,
    pathname,
  );
  ...
  if (
    matchPath({ path: workspaceRoutePaths.resumeGallery, end: true }, pathname)
  ) {
...
export function getWorkspaceViewFromRoute(
